...
void main()
{
    ...
    int windowSize = 80;//initially 2, grow from there
...
void loadResiduals(int f, int windowSize)
{
    ...
    writefln("calc residuals for f [%d - %d]", firstF+1, lastF+1);
    ...
    {
        //load feature loadF from file loadF+1
        featureFile = new File("features//" ~ std.string.toString(loadF+1) ~ "_c");
        f_c_load.length = 480189;
        for(i = 0; i < 480189; i++)
            featureFile.read(f_c_load[i]);
        featureFile.close();
        
        featureFile = new File("features//" ~ std.string.toString(loadF+1) ~ "_m");
